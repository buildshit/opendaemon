...
fn load_fixture(name: &str) -> DmnConfig {
    ...
    serde_json::from_str(&content).expect(&format!("Failed to parse fixture: {}", path.display()))
}
...
async fn test_simple_linear_startup_sequence() {
    ...
    assert!(
        wait_for_ready(&orchestrator, "database", 10).await,
        "Database not ready"
    );
    assert!(
        wait_for_ready(&orchestrator, "backend", 10).await,
        "Backend not ready"
    );
    assert!(
        wait_for_ready(&orchestrator, "frontend", 10).await,
        "Frontend not ready"
    );
    ...
    assert!(backend_logs
        .iter()
        .any(|l| l.content.contains("Backend listening")));
}
...
async fn test_graceful_shutdown() {
    ...
    assert!(matches!(
        status1,
        Some(ServiceStatus::Stopped) | Some(ServiceStatus::Failed { .. })
    ));
    assert!(matches!(
        status2,
        Some(ServiceStatus::Stopped) | Some(ServiceStatus::Failed { .. })
...
async fn test_env_file_loading() {
    ...
    let db_log_text = db_logs
        ...
        .join("\n");
    assert!(
        db_log_text.contains("DATABASE_URL") || db_log_text.contains("postgresql"),
        "Database logs should contain env var: {}",
        db_log_text
    );
    ...
    let api_log_text = api_logs
        ...
        .join("\n");
    assert!(
        api_log_text.contains("API_KEY") || api_log_text.contains("test_api_key"),
        "API logs should contain env var: {}",
        api_log_text
    );
}
...
async fn test_custom_timeout_configuration() {
    ...
    let result = orchestrator
        .start_service_with_deps("default_timeout_service")
        .await;
    assert!(
        result.is_ok(),
        "Default timeout service should start successfully"
    );
    ...
    assert!(
        wait_for_ready(&orchestrator, "quick_service", 5).await,
        "Quick service should be ready within custom timeout"
    );
    assert!(
        wait_for_ready(&orchestrator, "default_timeout_service", 5).await,
    ...
    assert!(
        quick_logs
            .iter()
            .any(|l| l.content.contains("Service ready")),
    ...
    assert!(
        default_logs
            .iter()
            .any(|l| l.content.contains("Default ready")),
