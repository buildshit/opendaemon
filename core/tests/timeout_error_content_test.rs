...
async fn test_log_timeout_error_message_contains_all_details() {
    ...
    // Check service name
    assert!(
        err_str.contains("test_service"),
        "Error should contain service name"
    );

    // Check timeout duration
    assert!(
        err_str.contains("0 seconds"),
        "Error should contain timeout duration"
    );

    // Check condition details
    assert!(
        err_str.contains("log_contains"),
        "Error should contain condition type"
    );
    assert!(
        err_str.contains("READY_SIGNAL"),
        "Error should contain pattern"
    );

    // Check that it includes log lines
    assert!(
        err_str.contains("Last") && err_str.contains("log lines"),
        "Error should mention log lines"
    );
    assert!(
        err_str.contains("Log line"),
        "Error should include actual log content"
    );

    // Check troubleshooting section
    assert!(
        err_str.contains("Troubleshooting"),
        "Error should contain troubleshooting section"
    );
    assert!(
        err_str.contains("timeout_seconds"),
        "Error should suggest timeout configuration"
    );
}
...
async fn test_url_timeout_error_message_contains_all_details() {
    ...
    assert!(
        err_str.contains("attempts"),
        "Error should mention connection attempts"
    );
    ...
    assert!(
        err_str.contains("listening"),
        "Error should suggest checking if service is listening"
    );
}
...
async fn test_log_timeout_with_no_logs_shows_appropriate_message() {
    ...
    assert!(
        err_str.contains("No log output received"),
        "Error should indicate no logs received"
    );
}
...
async fn test_log_timeout_captures_last_10_lines_only() {
    ...
    assert!(
        err_str.contains("Last 10 log lines"),
        "Error should indicate 10 log lines"
    );

    // Should contain lines 6-15 (last 10)
    assert!(
        err_str.contains("Log line 15"),
        "Error should contain last line"
    );
    assert!(
        err_str.contains("Log line 6"),
        "Error should contain 10th from last line"
    );
    ...
    assert!(
        !has_line_1,
        "Error should not contain 'Log line 1' as a separate line"
    );
    assert!(
        !has_line_5,
        "Error should not contain 'Log line 5' as a separate line"
    );
}
