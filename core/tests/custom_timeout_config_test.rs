...
async fn test_load_config_with_custom_timeouts() {
    ...
    let config: DmnConfig = serde_json::from_str(&config_content)
    ...
    let database = config.services.get("database").expect("database service should exist");
    if let Some(ready_when) = &database.ready_when {
        match ready_when {
            dmn_core::config::ReadyCondition::LogContains { pattern, timeout_seconds } => {
                assert_eq!(pattern, "Database ready");
                assert_eq!(*timeout_seconds, Some(10), "database should have 10 second timeout");
            }
    ...
    let api = config.services.get("api").expect("api service should exist");
    if let Some(ready_when) = &api.ready_when {
        match ready_when {
            dmn_core::config::ReadyCondition::LogContains { pattern, timeout_seconds } => {
                assert_eq!(pattern, "API listening");
                assert_eq!(*timeout_seconds, Some(15), "api should have 15 second timeout");
            }
    ...
    let web = config.services.get("web").expect("web service should exist");
    if let Some(ready_when) = &web.ready_when {
        match ready_when {
            dmn_core::config::ReadyCondition::UrlResponds { url, timeout_seconds } => {
                assert_eq!(url, "http://localhost:8000");
                assert_eq!(*timeout_seconds, Some(30), "web should have 30 second timeout");
            }
    ...
    let worker = config.services.get("worker").expect("worker service should exist");
    if let Some(ready_when) = &worker.ready_when {
        match ready_when {
            dmn_core::config::ReadyCondition::LogContains { pattern, timeout_seconds } => {
                assert_eq!(pattern, "Worker ready");
                assert_eq!(*timeout_seconds, None, "worker should not have custom timeout");
            }
...
async fn test_orchestrator_uses_custom_timeouts_from_config() {
    ...
    let mut config: DmnConfig = serde_json::from_str(&config_content)
        .expect("Should be able to parse config");
    ...
    assert!(elapsed < Duration::from_secs(10), 
        "Should timeout with custom 2 second timeout, took {:?}", elapsed);
}
    ...
    if let Some(ready_when) = &service.ready_when {
        match ready_when {
            dmn_core::config::ReadyCondition::LogContains { pattern, timeout_seconds } => {
                ...
                assert_eq!(*timeout_seconds, None, "Should default to None when not specified");
            }
...
async fn test_mixed_timeout_configurations() {
    ...
    let config: DmnConfig = serde_json::from_str(config_json)
        .expect("Should parse config with mixed timeouts");
    ...
    if let Some(dmn_core::config::ReadyCondition::LogContains { timeout_seconds, .. }) = &fast.ready_when {
    ...
    if let Some(dmn_core::config::ReadyCondition::LogContains { timeout_seconds, .. }) = &slow.ready_when {
    ...
    if let Some(dmn_core::config::ReadyCondition::LogContains { timeout_seconds, .. }) = &default.ready_when {
...
async fn test_url_responds_with_custom_timeout() {
    ...
    let config: DmnConfig = serde_json::from_str(config_json)
        .expect("Should parse config with url_responds timeout");
    ...
    assert!(elapsed < Duration::from_secs(10), 
        "Should timeout with custom 3 second timeout, took {:?}", elapsed);
}
