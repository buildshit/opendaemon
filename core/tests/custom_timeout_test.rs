...
async fn test_custom_timeout_log_contains_respected() {
    ...
    let result = orchestrator
        .start_service_with_deps("custom_timeout_service")
        .await;
    ...
    assert!(
        !orchestrator
            .is_service_ready("custom_timeout_service")
            .await
    );
    ...
    assert!(
        ...
        "Timeout should have occurred around 2 seconds, but took {:?}",
        elapsed
    );
}
...
async fn test_custom_timeout_url_responds_respected() {
    ...
    let start = Instant::now();
    let result = orchestrator
        .start_service_with_deps("api_custom_timeout")
        .await;
...
async fn test_custom_timeout_success_before_timeout() {
    ...
    assert!(
        orchestrator.is_service_ready("quick_service").await,
    ...
    assert!(
        ...
        "Service should become ready quickly, took {:?}",
        elapsed
    );
}
...
async fn test_very_short_custom_timeout() {
    ...
    let start = Instant::now();
    let result = orchestrator
        .start_service_with_deps("instant_timeout")
        .await;
